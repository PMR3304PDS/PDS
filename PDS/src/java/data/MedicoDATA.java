...
public class MedicoDATA {
  ...
  public void incluir(MedicoDO medico, Transacao tr) throws Exception {
     ...
     String sql = "insert into Medico (Conselhos_Con_cond, Estado_Est_cod_conselho_emissor, Med_NumRegistro) values (?, ?, ?)";
  ...
  public MedicoDO buscar(int idobj, Transacao tr) throws Exception {
     Connection con = tr.obterConexao();
     String sql = "select * from Medico where Conselhos_Con_cond=?";
     PreparedStatement ps = con.prepareStatement(sql);
     ps.setInt(1, idobj);
     ResultSet rs = ps.executeQuery();
     rs.next();
     MedicoDO medico = new MedicoDO();
     medico.setConselhos_Con_cond(rs.getInt("Conselhos_Con_cond"));
